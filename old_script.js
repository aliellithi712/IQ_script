...
if ((cellB == valuesO[i][0] && cellB != '') || (cellB == valuesN[i][0] && cellB != '')){
  ...
  var check = sheet.getRange("N" + selectedRow);
...
if (cellContent == ";" || cellContent == "ك" ) {
  ...
  // if (selectedColumn == 3)
  //   var counterCell = sheet.getRange("N" + selectedRow);
  // else  
  //   var counterCell = sheet.getRange("O" + selectedRow);
  // var counterValue = counterCell.getValue() || 0; // Get current value in L cell or 0 if empty
  // counterValue++;
  // counterCell.setValue(counterValue);
}
 else {
