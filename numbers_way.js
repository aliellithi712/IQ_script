...
var selectedRow = cellRange.getRow();
  var isDigitOnly = /^\d+$/.test(e.range.getValue());
...
/*  Change the empty and do not change the changed */
  ...
  if(selectedColumn != 5){
    ...
    else if (selectedColumn == 1 && e.oldValue !== 'none'){
      e.source.toast("You can't modify this cell");    
      e.range.setValue(e.oldValue);
    }
    ...
    else if((selectedColumn == 3 || selectedColumn == 4) && (e.oldValue != '-') ){
      var decimalTime = e.oldValue; 
      var totalSeconds = decimalTime * 86400;
    ...
    else if (selectedColumn == 2 && (e.range.getValue() === '' ||e.range.getValue() === "" || e.range.getValue() === null || !isDigitOnly) && e.oldValue === 'none') {
      e.source.toast("You can't delete this cell OR write a name");    
      e.range.setValue('none');
      return;
    }
    else if ((selectedColumn == 14 || selectedColumn == 15) && e.oldValue ==  '-'  && isDigitOnly){
      e.source.toast("Subscription will be inserted"); 
    }
    else{
      var newValue = e.value;
      var oldValue = e.oldValue;
      if(selectedColumn == 2){sheet.getRange("J" + selectedRow).setValue(sheet.getRange("A" + e.range.getValue()).getValue())}
...
if  (selectedColumn == 2 && cell.getValue()!= 'none'){
var substringg = "عليه";
